...
function TokenAmount({ amount, meta }: { amount: string; meta: TokenMetadata }) {
  ...
  return (
    <span
      className="cursor-pointer whitespace-nowrap font-mono hover:underline decoration-dotted"
      onClick={() => setShowRaw(!showRaw)}
      title={showRaw ? `Click to show in ${meta.symbol}` : "Click to show raw units"}
    >
      {showRaw
        ? `${amount} units ${meta.symbol}`
        : `${formatTokenAmount(amount, meta.decimals)} ${meta.symbol}`}
    </span>
  );
}
...
export default function TransferSummary({
  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-1">
      {transfer.from !== null ? (
        <AccountId accountId={transfer.from} />
      ) : (
        <SystemBadge label="Mint" />
      )}
      <span className="text-gray-400">&rarr;</span>
      {transfer.to !== null ? (
        <AccountId accountId={transfer.to} />
      ) : (
        <SystemBadge label="Burn" />
      )}
